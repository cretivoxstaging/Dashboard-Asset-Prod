...
/** Urutan kondisi: Good (paling atas) → Maintenance → Damaged */
...
function conditionSortKey(c: string | undefined): number {
  if (c == null || c === "") return 99;
  return CONDITION_ORDER[String(c).toLowerCase()] ?? 99;
}
...
export function AssetTableClient() {
  ...
  const rows = useMemo(() => {
    ...
    const cond = conditionFilter.trim();
    let filtered = list;
    if (q) {
    ...
    if (cond) {
      filtered = filtered.filter(
        (row) => String(row.condition ?? "").toLowerCase() === cond.toLowerCase(),
      );
    }
    return [...filtered].sort(
      (a, b) => conditionSortKey(a.condition) - conditionSortKey(b.condition),
    );
  ...
  // Reset to page 1 when search, condition filter, or pageSize changes
  useEffect(() => {
    setCurrentPage((p) => Math.min(p, totalPages));
  }, [totalPages, searchQuery, conditionFilter, pageSize]);
                ...
                onSubmit={async (e) => {
                  ...
                  try {
                    if (!form.item_name.trim())
                      throw new Error("Item name wajib diisi.");

                    const url = isEdit
                      ? `/api/asset/${encodeURIComponent(String(editId))}`
                      : "/api/asset";
                    ...
                    formData.append(
                      "qty_in_stock",
                      String(
                        Number.isFinite(Number(form.qty_in_stock))
                          ? Number(form.qty_in_stock)
                          : 0,
                      ),
                    );
                    ...
                    {form.picture && !pictureFile ? (
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs text-zinc-500">
                          Ganti dengan file baru di bawah
                        </span>
                      </div>
                    ) : null}
                    <input
                          ...
                          <div className="flex items-center gap-3">
                            {a.picture ? (
                              <button
                                type="button"
                                onClick={() =>
                                  setPreviewImage(a.picture ?? null)
                                }
                              ...
                              >
                                ...
                                />
                                <span
                                  className="hidden text-zinc-400"
                                  aria-hidden
                                >
                                  🖼
                                </span>
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setForm({
                                ...
                                qty_in_stock:
                                  typeof a.qty_in_stock === "number"
                                    ? String(a.qty_in_stock)
                                    : "",
